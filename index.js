...
module.exports = ({
  ...
  visit(markdownAST, `code`, node => {
    ...
    let showLanguageTxt = '';
    if (showLanguage) {
      showLanguageTxt += `<div class="syntax-lang-name">${languageName}</div>`;
    }
    ...
    node.value = `` + `<div class="${highlightClassName}" data-language="${languageName}">` + `${showLanguageTxt}<pre${numLinesStyle} class="${className}${numLinesClass}">` + `<code class="${className}">` + `${highlightCode(languageName, node.value, highlightLines, noInlineHighlight)}` + `</code>` + `${numLinesNumber}` + `</pre>` + `</div>`;
  });
